...
type_tourelle = "" # "arducam" ou "raspberry". Si autre chose, ne modifiera pas la position du moteur autofocus

#Initialisaiton

if type_tourelle == "arducam":

	Vmin_moteur_autofocus = cap.get(cv2.CAP_PROP_AUTOFOCUS_MINIMUM)
	Vmax_moteur_autofocus = cap.get(cv2.CAP_PROP_AUTOFOCUS_MAXIMUM)

elif type_tourelle == "raspberry":
	Vmin_moteur_autofocus = cap.get(cv2.CAP_PROP_AUTOFOCUS_MINIMUM)
	Vmax_moteur_autofocus = cap.get(cv2.CAP_PROP_AUTOFOCUS_MAXIMUM)
else :
...
print("Type de tourelle : ", type_tourelle, "\tVmin_moteur_autofocus = ", Vmin_moteur_autofocus, "\tVmax_moteur_autofocus = ", Vmax_moteur_autofocus)	
...
for pos_T in Position_tourelle:
	...
	for pos_M in range(Vmin_moteur_autofocus, Vmax_moteur_autofocus + pas_focus, pas_focus): #Recherche de la position de la mire sur l'image globale
		...
		fct.position_moteur_flou(cap, type_tourelle, pos_M)
